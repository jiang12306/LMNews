...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.sendBtn = [[UIButton alloc]initWithFrame:CGRectMake(spaceX, self.codeView.frame.origin.y + self.codeView.frame.size.height + spaceY, screenRect.size.width - spaceX * 2, labHeight)];
        ...
        self.switchBtn = [[UIButton alloc]initWithFrame:CGRectMake((screenRect.size.width - 100) / 2, self.sendBtn.frame.origin.y + self.sendBtn.frame.size.height + spaceY, 100, 20)];
        [self.switchBtn setTitleColor:[UIColor colorWithHex:themeOrangeString] forState:UIControlStateNormal];
        [self.switchBtn setTitle:@"密码登录" forState:UIControlStateNormal];
        [self.switchBtn setTitle:@"验证码登录" forState:UIControlStateSelected];
        ...
        self.weChatBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, screenRect.size.height - 100, 80, 100)];
        [self.weChatBtn addTarget:self action:@selector(clickedWeChatButton:) forControlEvents:UIControlEventTouchUpInside];
        self.weChatBtn.center = CGPointMake(screenRect.size.width / 3, self.switchBtn.frame.origin.y + self.switchBtn.frame.size.height + spaceY + 50);
        ...
        self.qqBtn.center = CGPointMake(screenRect.size.width * 2 / 3, self.weChatBtn.center.y);
...
-(void)keyboardWillShow:(NSNotification* )notify {
    ...
    [UIView animateWithDuration:duration animations:^{
        self.contentView.frame = CGRectMake(0, screenSize.height - keyboardHeight - 170, screenSize.width, screenSize.height);
    }];
}
...
-(void)keyboardWillHide:(NSNotification* )notify {
    ...
    CGFloat duration = [[info objectForKey:UIKeyboardAnimationDurationUserInfoKey]floatValue];
    CGSize screenSize = [UIScreen mainScreen].bounds.size;
    CGFloat contentHeight = self.weChatBtn.frame.origin.y + self.weChatBtn.frame.size.height + 20;
    [UIView animateWithDuration:duration animations:^{
